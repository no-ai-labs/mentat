    ...
    <div class="block-card data-block">
      ...
      <div class="space-y-4">
        ...
        <div v-if="parsedContent && ['csv', 'tsv', 'json', 'xlsx'].includes(parsedContent.format)" class="preview-section">
          ...
          <!-- 미리보기 콘텐츠 -->
          <div 
            class="preview-content nodrag nozoom"
            @wheel="handleWheel"
          >
...
// 로컬 상태
const localData = ref(props.data.rawContent || '')
const parsedContent = ref(props.data.parsedContent || null)
...
const updateData = () => {
  if (props.data) {
    props.data.rawContent = localData.value
    props.data.parsedContent = parsedContent.value
  }
}
...
// 스크롤 이벤트 핸들러 (Vue Flow 줌 방지)
const handleWheel = (event) => {
  // Ctrl/Cmd + 휠이면 줌이므로 기본 동작 허용
  if (event.ctrlKey || event.metaKey) {
    return
  }
  
  // 일반 스크롤이면 Vue Flow로 전파 방지
  event.stopPropagation()
}

// 초기 데이터 로드
watch(() => props.data.parsedContent, (newContent) => {
  if (newContent) {
